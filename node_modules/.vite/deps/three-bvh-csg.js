import { $r as MathUtils, Bo as SphereGeometry, Bs as Vector3, Dr as Line3, J as BufferAttribute, Nr as LineSegments, Or as LineBasicMaterial, Qn as InstancedMesh, Vs as Vector4, W as Box3, Y as BufferGeometry, _s as Triangle, ai as MeshBasicMaterial, ct as Color, di as MeshPhongMaterial, ii as Mesh, lo as Ray, ni as Matrix4, ra as Plane, ti as Matrix3, zn as Group, zs as Vector2 } from "./three.module-C5Sn_aNr.js";
...
var MeshBVH = class MeshBVH extends GeometryBVH {
	...
	raycastObject3D(object, raycaster, intersects = []) {
		...
		_ray$3.copy(raycaster.ray).applyMatrix4(_inverseMatrix$1);
		_worldScale.setFromMatrixScale(object.matrixWorld);
		_direction.copy(_ray$3.direction).multiply(_worldScale);
		const scaleFactor = _direction.length();
		const near = raycaster.near / scaleFactor;
		const far = raycaster.far / scaleFactor;
		if (raycaster.firstHitOnly === true) {
			let hit = this.raycastFirst(_ray$3, material, near, far);
			hit = convertRaycastIntersect(hit, object, raycaster);
			if (hit) intersects.push(hit);
		} else {
			const hits = this.raycast(_ray$3, material, near, far);
...
function computeDisjointEdges(geometry, unmatchedSet, eps) {
	...
	for (let i = 0, l = edges.length; i < l; i++) {
		...
		if (_ray$2.direction.dot(commonRay.direction) < 0) info.reverse.push({
			start,
			end,
			index
		});
...
var require_orientation = __commonJS({ "node_modules/robust-orientation/orientation.js"(exports, module) {
	...
	var CACHED = [
		...
		function orientation3(a, b, c) {
			...
			var det = l - r;
			var s;
			if (l > 0) if (r <= 0) return det;
			else s = l + r;
			else if (l < 0) if (r >= 0) return det;
			else s = -(l + r);
			else return det;
			var tol = ERRBOUND3 * s;
...
var require_filter = __commonJS({ "node_modules/cdt2d/lib/filter.js"(exports, module) {
	...
	function classifyFaces(triangulation, target, infinity) {
		...
		if (target === 0) if (infinity) return index.cells.concat(index.boundary);
		...
		while (active.length > 0 || next.length > 0) {
			while (active.length > 0) {
				...
				for (var j = 0; j < 3; ++j) {
					...
					if (f >= 0 && flags[f] === 0) if (constraint[3 * t + j]) next.push(f);
					else {
						active.push(f);
						flags[f] = side;
					}
...
var _vec$2 = new Vector3();
var _vec2$1 = new Vector3();
...
function edgesToIndices(edges, outputVertices, outputIndices, epsilonScale) {
	...
	for (let i = 0, l = edges.length; i < l; i++) {
		...
		for (let i1 = i + 1; i1 < l; i1++) {
			...
			if (edge0.distanceSqToLine3(edge1, _vec$2, _vec2$1) < RELATIVE_EPSILON * epsilonScale) getIndex(_vec2$1);
		}
	}
	const arr = [];
	for (let i = 0, l = edges.length; i < l; i++) {
...
var LegacyTriangleSplitter = class {
	...
	initialize(tri) {
		...
		if (Array.isArray(tri)) for (let i = 0, l = tri.length; i < l; i++) {
			...
			else if (Math.abs(1 - t.getNormal(_vec$1).dot(normal)) > EPSILON) throw new Error("Triangle Splitter: Cannot initialize with triangles that have different normals.");
	...
	splitByPlane(plane, clippingTriangle) {
		...
		for (let i = 0, l = triangles.length; i < l; i++) {
			...
			if (!_splittingTriangle.intersectsTriangle(tri, _edge$2, true)) continue;
			const { a, b, c } = tri;
			let intersects = 0;
			...
			for (let t = 0; t < 3; t++) {
				...
				if (didIntersect && !(_vec$1.distanceTo(_edge$2.start) < EPSILON)) {
					if (_vec$1.distanceTo(_edge$2.end) < EPSILON) vertexSplitEnd = t;
					if (intersects === 0) _foundEdge.start.copy(_vec$1);
					else _foundEdge.end.copy(_vec$1);
					intersects++;
				}
			}
			if (!coplanarEdge && intersects === 2 && _foundEdge.distance() > COPLANAR_EPSILON) if (vertexSplitEnd !== -1) {
...
//#endregion
//#region node_modules/three-bvh-csg/src/core/constants.js
var ADDITION = 0;
var SUBTRACTION = 1;
...
function clipSegmentToTriangle(segment, tri, normal, target) {
	...
	for (let i = 0; i < 3; i++) {
		...
		if (Math.abs(denom) < PARALLEL_EPSILON) if (dist < -CLIP_EPSILON) return null;
		else continue;
		const t = -dist / denom;
...
function getHitSide(tri, bvh, matrix = null) {
	...
	const hit = bvh.raycastFirst(_ray$1, 2);
	return Boolean(hit && _ray$1.direction.dot(hit.face.normal) > 0) ? -1 : 1;
}
function collectIntersectingTriangles(a, b) {
	...
	a.geometry.boundsTree.bvhcast(b.geometry.boundsTree, _matrix$2, { intersectsTriangles(triangleA, triangleB, ia, ib) {
		if (!isTriDegenerate(triangleA) && !isTriDegenerate(triangleB)) {
			...
			if (isCoplanarIntersection || triangleA.intersectsTriangle(triangleB, _edge$1, true)) {
...
function performSplitTriangleOperations(a, b, intersectionMap, operations, invert, splitter, builders, groupOffset = 0) {
	...
	_inverseMatrix.copy(_matrix$1).invert();
	if (invert) _builderMatrix.copy(_matrix$1);
		...
		for (let ib = 0, l = triangles.length; ib < l; ib++) {
			...
			for (let cp = 0, cpl = _coplanarTriangles.length; cp < cpl; cp++) {
				...
				if (cpt.containsPoint(_midpoint)) {
					...
					hitSide = _normal$1.dot(_coplanarNormal) > 0 ? 2 : -2;
					break;
				}
			}
	...
	for (let id = 0; id < triCount; id++) {
		...
		while (stack.length > 0) {
			...
			if (_builders.length !== 0) {
				...
				_tri$1.b.fromBufferAttribute(aPosition, i1);
				_tri$1.c.fromBufferAttribute(aPosition, i2);
				if (!isTriDegenerate(_tri$1)) for (let k = 0, lk = _builders.length; k < lk; k++) {
					const builder = _builders[k];
...
//#endregion
//#region node_modules/three-bvh-csg/src/core/operations/Operation.js
...
//#endregion
//#region node_modules/three-bvh-csg/src/core/operations/OperationGroup.js
...
//#endregion
//#region node_modules/three-bvh-csg/src/materials/shaderUtils.js
function addWorldPosition(shader) {
	if (/varying\s+vec3\s+wPosition/.test(shader.vertexShader)) return;
	shader.vertexShader = `
			varying vec3 wPosition;
			${shader.vertexShader}
		`.replace(/#include <displacementmap_vertex>/, (v) => `${v}
...
function csgGridShaderMixin(shader) {
	...
	shader.fragmentShader = shader.fragmentShader.replace(/#include <common>/, (v) => `
			${v}
		...
		`).replace(/#include <normal_fragment_maps>/, (v) => `${v}
				#if CSG_GRID
				{
...
//#endregion
//#region node_modules/three-bvh-csg/src/materials/GridMaterial.js
...
//#endregion
//#region node_modules/three-bvh-csg/src/core/debug/debugUtils.js
...
function generateRandomTriangleColors(geometry) {
	...
	for (let i = 0, l = array.length; i < l; i += 9) {
		color.setHSL(Math.random(), MathUtils.lerp(.5, 1, Math.random()), MathUtils.lerp(.5, .75, Math.random()));
		array[i + 0] = color.r;
		array[i + 1] = color.g;
		array[i + 2] = color.b;
		array[i + 3] = color.r;
...
//#endregion
//#region node_modules/three-bvh-csg/src/objects/TriangleSetHelper.js
var TriangleSetHelper = class extends Group {
	...
	setTriangles(triangles) {
		const triPositions = new Float32Array(9 * triangles.length);
		const linePositions = new Float32Array(18 * triangles.length);
		for (let i = 0, l = triangles.length; i < l; i++) {
			const i9 = 9 * i;
			const i18 = 18 * i;
			const tri = triangles[i];
...
//#endregion
//#region node_modules/three-bvh-csg/src/objects/EdgesHelper.js
...
//#endregion
//#region node_modules/three-bvh-csg/src/objects/PointsHelper.js
...
var PointsHelper = class extends InstancedMesh {
	...
	constructor(count = 1e3, points = []) {
		super(new SphereGeometry(.025), new MeshBasicMaterial(), count);
		this.frustumCulled = false;
...
//#endregion
//#region node_modules/three-bvh-csg/src/objects/HalfEdgeHelper.js
var vertKeys = [
	"a",
	"b",
	"c"
...
var HalfEdgeHelper = class extends EdgesHelper {
	constructor(geometry = null, halfEdges = null) {
		...
		if (geometry && halfEdges) this.setHalfEdges(geometry, halfEdges);
	}
	setHalfEdges(geometry, halfEdges) {
		...
		let triCount = getTriCount(geometry);
		if (geometry.drawRange.count !== Infinity) triCount = ~~(geometry.drawRange.count / 3);
		if (displayDisconnectedEdges) if (halfEdges.unmatchedDisjointEdges) halfEdges.unmatchedDisjointEdges.forEach(({ forward, reverse, ray }) => {
		...
		else for (let triIndex = offset; triIndex < triCount; triIndex++) {
			getTriangle(geometry, triIndex, _tri1);
			for (let e = 0; e < 3; e++) if (halfEdges.getSiblingTriangleIndex(triIndex, e) === -1) {
				const nextE = (e + 1) % 3;
			...
			for (let e = 0; e < 3; e++) {
				...
				if (otherTriIndex === -1) continue;
				getTriangle(geometry, otherTriIndex, _tri2);
				const nextE = (e + 1) % 3;
				const v0 = _tri1[vertKeys[e]];
				const v1 = _tri1[vertKeys[nextE]];
				_centerPoint.lerpVectors(v0, v1, .5);
				addConnectionEdge(_tri1, _tri2, _centerPoint);
			}
			if (halfEdges.disjointConnections) for (let e = 0; e < 3; e++) {
				...
				for (let i = 0; i < disjointTriIndices.length; i++) {
					...
					_centerPoint.lerpVectors(_edge.start, _edge.end, .5);
					addConnectionEdge(_tri1, _tri2, _centerPoint);
				}
			}
		}
		...
		function addConnectionEdge(tri1, tri2, centerPoint) {
			...
			if (straightEdges) {
				...
				if (_projectedDir.dot(_edgeDir) < 0) _projectedDir.multiplyScalar(-1);
				if (_projectedDir2.dot(_edgeDir2) < 0) _projectedDir2.multiplyScalar(-1);
				...
				edge.end.lerpVectors(_finalPoint, _finalPoint2, .5);
			} else edge.end.copy(centerPoint);
			edges.push(edge);
		}
	}
};
//#endregion
//#region node_modules/three-bvh-csg/src/utils/computeMeshVolume.js
